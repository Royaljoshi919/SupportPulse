                 ...
                 await Task.Delay(4000, stoppingToken);
                      ...
                      else
                       {
                         ...
                         throw new Exception($"Gemini Vision API Error ({response.StatusCode}): {errStr}");
                          ...
                          //  ==========================================
// 🔍 STEP 4: KNOWLEDGE BASE SEARCH (RAG)
// ==========================================
_logger.LogInformation($"[Job #{job.Id}] Searching Knowledge Base for Category: {classification.Category}");

var kbArticle = await dbContext.KnowledgeBaseArticles
    .Where(k => k.Category.ToLower() == classification.Category.ToLower())
    .FirstOrDefaultAsync(stoppingToken);

string policyContext = kbArticle != null 
    ? kbArticle.Content 
    : "No specific company policy found. Reply politely and state that a human agent will assist soon.";

string ragPrompt = $@"
You are a helpful and professional customer support agent.
Generate a polite suggested reply to the customer's ticket.

CRITICAL RULES:
1. You MUST base your reply ONLY on the 'Company Policy' provided below.
2. DO NOT invent, assume, or hallucinate any rules, timeframes, or policies outside of the given text.
3. Keep the response concise, empathetic, and professional.

Company Policy Context:
{policyContext}

Customer's Ticket Issue:
{ticket.Description}
";

_logger.LogInformation($"[Job #{job.Id}] Generating Grounded Suggested Response...");
string suggestedResponse = string.IsNullOrWhiteSpace(geminiKey)
    ? "A human support agent will review your ticket and assist you soon."
    : await CallGeminiForTextAsync(ragPrompt, httpClient, geminiKey, stoppingToken);

// ==========================================
// 💾 STEP 5: SAVE ALL AI RESULTS TO DATABASE
// ==========================================
// Check karein ki kya is TicketId ka record pehle se DB mein hai
var existingAiResult = await dbContext.TicketAiResults
    .FirstOrDefaultAsync(x => x.TicketId == job.TicketId, stoppingToken);

if (existingAiResult != null)
{
    // Agar pehle se record exist karta hai, toh update karein
    existingAiResult.Transcription = audioResultText;
    existingAiResult.ImageDescription = visionResultText;
    existingAiResult.SuggestedResponse = suggestedResponse;
}
else
{
    // Agar record nahi hai, toh naya bana kar add karein
    var newAiResult = new TicketAiResult
    {
        TicketId = job.TicketId,
        Transcription = audioResultText,
        ImageDescription = visionResultText,
    ...
    dbContext.TicketAiResults.Add(newAiResult);
}
...
// Single Database Save Call
await dbContext.SaveChangesAsync(stoppingToken);
_logger.LogInformation($"[Job #{job.Id}] All AI Results & Ticket updated successfully!");
                            ...
                            await dbContext.SaveChangesAsync(stoppingToken);
                          _logger.LogInformation("Waiting 3 seconds to avoid API Rate Limits (429 Error)...");
        ...
        private async Task<string> CallGeminiForTextAsync(string prompt, HttpClient httpClient, string apiKey, CancellationToken token)
        {
            var payload = new
            {
                contents = new[]
            ...
            var jsonContent = new StringContent(JsonSerializer.Serialize(payload), Encoding.UTF8, "application/json");
            string url = $"https://generativelanguage.googleapis.com/v1beta/models/gemini-3.8-flash:generateContent?key={apiKey}";

            var response = await httpClient.PostAsync(url, jsonContent, token);
        ...
        */
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            _logger.LogInformation("🚀 AI Enrichment Background Worker started (Powered by Groq).");
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (job != null)
                    {
                        ...
                        try
                        {
                            // Dono text aur audio ke liye Groq Key ka use
                            var groqApiKey = _configuration["GroqSettings:ApiKey"] ?? _configuration["OpenAISettings:ApiKey"];
                            ...
                            // ==========================================
                            // 🎧 SCENARIO 1: AUDIO FILE PROCESSING (GROQ WHISPER)
                            // ==========================================
                            if (audioFile != null)
                            {
                                string fullAudioPath = ResolveFilePath(audioFile.FilePath);
     ...
     // ==========================================
// 🖼️ SCENARIO 2: IMAGE PROCESSING (GEMINI 1.5 FLASH - FREE)
// ==========================================
if (imageFile != null)
{
    try
    {
        ...
        {
            _logger.LogWarning($"[Job #{job.Id}] Gemini API Key missing hai. Image analysis skip ho raha hai.");
        }
        else
        {
            string fullImagePath = ResolveFilePath(imageFile.FilePath);
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _logger.LogInformation($"[Job #{job.Id}] Gemini Image analysis successful!");
            }
            else
            {
                var errStr = await response.Content.ReadAsStringAsync(stoppingToken);
                _logger.LogError($"[Job #{job.Id}] Gemini Vision API Error ({response.StatusCode}): {errStr}");
            }
        }
    }
    catch (Exception ex)
    {
        _logger.LogError(ex, $"[Job #{job.Id}] Image processing skipped due to error: {ex.Message}");
    }
}
                            ...
                            // ==========================================
                            // 🏷️ STEP 3: FINAL CLASSIFICATION (GROQ gpt-oss-120b)
                            // ==========================================
                            _logger.LogInformation($"[Job #{job.Id}] Final AI Classification started...");
                            ...
                            // ==========================================
                            // 🔍 STEP 4: KNOWLEDGE BASE SEARCH & RAG (GROQ gpt-oss-120b)
                            // ==========================================
                            _logger.LogInformation($"[Job #{job.Id}] Searching Knowledge Base for Category: {classification.Category}");
...
";
                            ...
                            _logger.LogInformation($"[Job #{job.Id}] Generating Grounded Suggested Response...");
                            string suggestedResponse = await CallGroqForTextAsync(ragPrompt, httpClient, groqApiKey, stoppingToken);
        ...
        // Helper Method: AI Classification with Groq
        private async Task<TicketClassificationResult> ClassifyTicketAsync(
            string? ticketDescription,
            string? audioTranscription,
            string? imageDescription,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var payload = new
            {
                model = "openai/gpt-oss-120b", // User's strict requested model
                response_format = new { type = "json_object" },
                messages = new[]
                {
                    new { role = "system", content = "You must output JSON only." },
            ...
            // JSON format clean up in case Groq adds markdown blocks
        ...
        // Helper Method: RAG text generation using Groq
        private async Task<string> CallGroqForTextAsync(string prompt, HttpClient httpClient, string apiKey, CancellationToken token)
        {
            var payload = new
            {
                model = "openai/gpt-oss-120b", // User's strict requested model
